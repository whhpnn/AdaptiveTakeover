...
# ===================== 1. 轨迹预测专用参数配置 =====================
DATA_DIR = r"/home/kenway/wahaha/PycharmProjects/trajectory_prediction/data"
...
# ========== 关键修改：特征列和目标列定义 ==========
# FEATURE_COLS：2-6列（Excel列索引从0开始，对应索引1,2,3,4,5）
# TARGET_COLS：第二列和第三列（对应索引1,2）
FEATURE_COLS = [1, 2, 3, 4, 5]  # 第2-6列（5维特征）
TARGET_COLS = [1, 2]  # 目标：第二列和第三列
print(f"使用设备: {DEVICE}")
print(f"CUDA设备名称: {torch.cuda.get_device_name(DEVICE) if torch.cuda.is_available() else '无'}")
print(f"特征列（Excel第2-6列）: 索引{FEATURE_COLS}")
...
class TrajectoryDataset(Dataset):
    def __init__(self, input_sequences, target_sequences):
        self.inputs = input_sequences  # (n_samples, 40, 5)
