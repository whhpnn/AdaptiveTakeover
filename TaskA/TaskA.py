...
DATA_DIR = r"D:\\software\CARLA_0.9.11\WindowsNoEditor\wrs_demo\overtaking\data" # 替换为你的路径
...
HIDDEN_SIZE = 128
NUM_LAYERS = 2
MODEL_PATH = r"D:\\software\CARLA_0.9.11\WindowsNoEditor\wrs_demo\overtaking\lstm_trajectory_predictor_gpu.pth"  # 训练好的模型权重路径
# 2.4 设备配置
...
class World(object):
    ...
    def restart(self):
        ...
        while self.player is None:
            ...
            spawn_point = carla.Transform(carla.Location(x=203.268661, y=-350.982361, z=1),
                                          carla.Rotation(yaw=92.314423))
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
            self.modify_vehicle_physics(self.player)
            self.player.enable_constant_velocity(carla.Vector3D(x=5, y=0.0, z=0.0))

            spawn_pointobstacle1 = carla.Transform(carla.Location(x=200, y=-250+np.random.uniform(-5, 5), z=1), carla.Rotation(yaw=90))


            self.obstacle1 = self.world.try_spawn_actor(blueprint, spawn_pointobstacle1)
            location = carla.Location(x=199, y=-220, z=1)
            # 大约八十米处任务结束，设置终点标记
...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds, world, delay):
        ...
        length = 2 * bounding_box.extent.x  # 车辆长度
        width = 2 * bounding_box.extent.y  # 车辆宽度
        ...
        dist = np.sqrt((x - world.obstacle1.get_location().x) ** 2 + (y - world.obstacle1.get_location().y) ** 2)
        arr = [x,y, transform, v0, dist,None]
        ...
        for i in range(len(steerCmd)):
            if steerCmd[i][0] != None:
                ...
                if (steerCmd[i][1] - fbdelay) != None and steerCmd[i][1] - fbdelay > 0 and self.takeover == 1:
                    self.statelist[steerCmd[i][1] - 1 - fbdelay][5] = self._steer_cache



        P_real =  self.calculate_P(dist)
        ...
        if self.takeover == 0:
            ...
            self.statelist[len(self.statelist)-1][5] = steercache_lanekeeping
            x_predicted = x
            y_predicted = y
            the_predicted = the
            for i in range(16):
                ...
                the_predicted = the_predicted + 0.05 * v0 * np.tan(
                    -self.lanekeeping(x_predicted, y_predicted, the_predicted, v0)) / 2.7
                dist_predicted = np.sqrt(
                    (x_predicted - world.obstacle1.get_location().x) ** 2 + (y_predicted - world.obstacle1.get_location().y) ** 2)
                P_predicted = self.calculate_P(dist_predicted)
                if P_predicted > 2:
                    self.TOR = 1
                    break
            #接管开始
            ...
            if len(self.statelist) > (40+16):
                ...
                for i in range(16):
                    ...
                    # 检查当前位置的d值是否为None
                    if self.statelist[current_index][5] is None:
            ...
            dist_car = self.min_distance_to_car(x, y, world.obstacle1.get_location().x,
                                                world.obstacle1.get_location().y, length, width)
            dist2 = np.abs((43.48 * x + y - 8390.1) / np.sqrt(43.48 ** 2 + 1 ** 2))
            dist3 = np.abs((43.48 * x + y - 8685.75) / np.sqrt(43.48 ** 2 + 1 ** 2))
            self.MAD.append(min(dist_car, dist2, dist3))
    ...
    def mpccontrol(self, x0, y0, v0, the0, intended_x, intended_y, ulast0):
        ...
        def cost_function(u):
            ...
            for i in range(4):
                ...
                the = the + 0.05 * v0 * np.tan(u[i]) / 2.7
                detau = u[i] - ulast
                cost += 6 * (x - intended_x[i]) ** 2 + 4 * (y - intended_y[i]) ** 2 + 1 * u[i] ** 2 + 0.5*detau**2  # 代价函数
                ulast = u[i]
            return cost
    ...
    def min_distance_to_car(self, x, y, a, b, length, width):
        """
        计算平面内点(x,y)到矩形车辆边缘的最小距离（车辆无旋转，中心在(a,b)，长length，宽width）
        ...
        返回:
            float: 点到车辆边缘的最小距离（单位与输入坐标一致）
        """
        # 1. 计算车辆矩形的四个边界
        x_left = a - width / 2  # 车辆左边界x
        x_right = a + width / 2  # 车辆右边界x
        y_bottom = b - length / 2  # 车辆下边界y
        y_top = b + length / 2  # 车辆上边界y

        # 2. 情况1：点在车辆内部/边缘 → 最小距离为0
        if (x_left <= x <= x_right) and (y_bottom <= y <= y_top):
            return 0.0

        # 3. 情况2：点在车辆四条边的正外侧（投影在边上）
        # 正上方/正下方
        if x_left <= x <= x_right:
            if y > y_top:
                return y - y_top
        ...
        # 4. 情况3：点在车辆四角外侧 → 计算到最近角点的欧式距离
        # 车辆四个角点坐标
        corners = [
            (x_left, y_top),  # 左上角
            (x_right, y_top),  # 右上角
            (x_left, y_bottom),  # 左下角
            (x_right, y_bottom)  # 右下角
        ]
        # 计算到每个角点的距离，取最小值
        min_dist = float('inf')
        for cx, cy in corners:
            dist = math.hypot(x - cx, y - cy)  # 等价于sqrt((x-cx)² + (y-cy)²)，更高效
            if dist < min_dist:
                min_dist = dist
        return min_dist

    def calculate_P(self, dist):
    ...
    def lanekeeping(self, x0, y0, the0, v0):
        ey = (-43.48 * x0 + 8464 - y0) / np.sqrt(1 ** 2 + 43.48 ** 2)
        u1 = -1.32 * np.pi / 180 - the0 + np.arctan(1.8 * ey / v0)
...
# -- game_loop() ---------------------------------------------------------------
# ==============================================================================
def get_valid_delay(data, upper, lower=0):
    while True:
        delay = int(1000 * np.random.choice(data) / 50)
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            delaycontrol = get_valid_delay(datacontrol, upper=6)
            delayfeedback = 10
            world.world.tick()
