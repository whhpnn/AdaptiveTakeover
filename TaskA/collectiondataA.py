...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, world):

        numAxes = self._joystick.get_numaxes()
        jsInputs = [float(self._joystick.get_axis(i)) for i in range(numAxes)]
        T = jsInputs[self._steer_idx]
        K1 = 1 # 0.55
        steerCmd = K1 * math.tan(1.1 * T)
        ...
        uh =  self._control.steer
        arr = [x,y, transform, v0, dist,uh]
        if y < -200:
...
def game_loop(args):
    ...
    try:
        ...
        pygame.quit()
        dataframe = pd.DataFrame(controller.statelist)
        # 修正后代码
        dataframe.to_excel('D:\software\CARLA_0.9.11\WindowsNoEditor\wrs_demo\overtaking\\traindata\\5ms_01.xlsx')
